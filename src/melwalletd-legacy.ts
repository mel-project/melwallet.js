...
import { MelwalletdProtocol } from './melwalletd-prot';
...
export interface MelwalletdEndpoint {
  path: string[];
  method: HTTPMethod;
  body?: BodyInit;
}

export let melwalletd_endpoints = {
...
export class MelwalletdHttpClient {
  ...
  async request(
    ...
    return MelwalletdHttpClient.request(
      this.#base_url,
      endpoint,
      body_or,
      additional_options,
   ...
   * @returns {Promise<MelwalletdHttpWallet>}
   */
  async get_wallet(wallet_name: string): Promise<MelwalletdHttpWallet> {
    ...
    if (summary?.address) {
      ...
      return new MelwalletdHttpWallet(address, name, summary.network, this);
    }
...
export class MelwalletdHttpWallet implements ThemelioWallet {
  readonly #address: string;
  readonly #client: MelwalletdHttpClient;
  ...
  async get_transaction_balance(txhash: string): Promise<TxBalance> {
    ...
    let raw_balances: [string, bigint][] = Object.entries(
      raw_balance[2] as Record<string, bigint>,
    );

    let balances: [Denom, bigint][] = raw_balances.map(([denom, value]) => {

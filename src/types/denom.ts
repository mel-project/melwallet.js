...
export class Denom {
  ...
  static fromString(s: string): Denom {
    ...
    if (splitted.length != 2 || splitted[0] != 'CUSTOM') {
      throw `Illegal argument passed to fromString(): ${s} does not correspond to any instance of the Denom ${
  ...
  private constructor(value: DenomNum) {
    this.value = value;
  }
  ...
  public toString(): DenomName {
    ...
    let name: `CUSTOM-${string}` = `CUSTOM-${this.value
      .toString(16)
      .toUpperCase()}`;
    return name;
  }

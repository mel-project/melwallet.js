...
import { WalletInfo, Wallet } from './themelio-wallet'
...
class ThemelioWallet implements Wallet {
  ...
  async summarize_wallet(): Promise<WalletSummary> {
    let wallet = this
    let summary = await fetch_wrapper(`${wallet.#endpoint}/wallet/${wallet.name}`)
    if (is<WalletSummary>(summary)) return summary as WalletSummary
...
async function fetch_wrapper(endpoint: RequestInfo, data?: RequestInit) {
  data = data || {}
